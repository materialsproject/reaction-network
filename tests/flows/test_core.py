...
def test_synthesis_planning_flow_job(synthesis_planning_flow_job, job_store):
    ...
    output = run_locally(synthesis_planning_flow_job, store=job_store, ensure_success=True)

    competition_doc = output[synthesis_planning_flow_job.job_uuids[-1]][1].output
    assert competition_doc is not None


def test_synthesis_planning_flow_additional_elems_job(synthesis_planning_flow_additional_elems_job, job_store):
    ...
    competition_doc = output[synthesis_planning_flow_additional_elems_job.job_uuids[-1]][1].output
    assert competition_doc is not None
...
def test_synthesis_planning_flow_open_job(synthesis_planning_flow_open_job, job_store):
    ...
    output = run_locally(synthesis_planning_flow_open_job, store=job_store, ensure_success=True)
